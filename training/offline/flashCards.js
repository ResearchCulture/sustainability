modelfilestrs['flashCards'] = hereDoc(function(){/*!
	...
	var flashCards = new function () {
		...
		this.sizeChanged = function(firstLoad) {
			...
			if (cardHeight == 'default') {
				...
				var totalSpace = $x_pageHolder.height() - (parseInt($x_pageDiv.css('padding-top')) * 2);
				
				if (x_browserInfo.mobile == true) {
					totalSpace = $x_mobileScroll.height() - $x_headerBlock.height() - $x_footerBlock.height() - (parseInt($x_pageDiv.css("padding-top")) * 2) - 3;
				}
		...
		this.setUp = function() {
			...
			$(cardArray).each(function(i){
				...
				$thisFront.add($thisBack).each(function(j) {
					...
					if (thisInfo.getAttribute('name') != '') {
						$thisInner.append('<h3>' + thisInfo.getAttribute('name') + '</h3>');
					}
            ...
            $('.flipBtn')
				...
				.click(function() {
					...
					// scroll to top the side that's about to come into view
					var hiddenSide = $(this).closest('.card').data("flip-model").isFlipped == true ? 0 : 1;
					$(this).closest('.card').find('.panel:eq(' + hiddenSide + ') .cardInner').scrollTop(0);
