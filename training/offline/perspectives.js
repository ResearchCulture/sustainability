modelfilestrs['perspectives'] = hereDoc(function(){/*!
	...
	var perspectives = new function() {
			...
			setTimeout(function(){
				...
				if (x_currentPageXML.getAttribute("thumbType") == "both" || x_currentPageXML.getAttribute("thumbType") == "imgNum" || x_currentPageXML.getAttribute("thumbType") == undefined) {
					$("#thumbBar .thumbBox").each(function() {
						...
						} else if ($this.find(".thumbMedia video").length > 0) {
						}
					});
				}
		...
		this.setUp = function() {
			...
			$("#prevBtn")
				.button({
					...
					label: x_currentPageXML.getAttribute("backBtnTip") == undefined ? "Back" : x_currentPageXML.getAttribute("backBtnTip"),
					text: false
				})
				.click(function() {
					var scrollL = 0;
			...
			$("#nextBtn")
				.button({
					...
					label: x_currentPageXML.getAttribute("nextBtnTip") == undefined ? "Forward" : x_currentPageXML.getAttribute("nextBtnTip"),
					text: false
				})
				.click(function() {
			...
			if (x_currentPageXML.getAttribute("thumbPos") == "grid" && x_currentPageXML.getAttribute("target") != "lightbox") {
				$gridBack
					...
					.click(function() {
						...
						$("#thumbBar .current").removeClass("current");
						
						$("#mainInfoHolder")
							.data("index", "undefined")
							.fadeOut(400, function() { $thumbBar.fadeIn(400); });
					});
			} else {
				$gridBack.hide();
			}
			...
			if (x_currentPageXML.getAttribute("playAll") == "true" && (x_currentPageXML.getAttribute("thumbPos") != "grid" || x_currentPageXML.getAttribute("target") != "lightbox")) {
				...
				$thisThumb
					.addClass("playAll")
					.find(".thumbTitle").html(x_currentPageXML.getAttribute("playTip") == undefined ? "Play All" : x_currentPageXML.getAttribute("playTip"));
			...
			$(x_currentPageXML).children().each(function(i) {
				if ((this.getAttribute("src") != undefined && this.getAttribute("src") != "") || this.nodeName == "mpText") {
					...
					if (thumbType == "both" || thumbType == "image" || thumbType == "imgNum") {
						...
						var maxImgSize = [$thisThumb.width(), $thisThumb.height() - $thisThumb.find(".thumbTitle").height() - 5];
						
						if (thumbType == "both") {
							$thisThumb.find(".thumbTitle").html(this.getAttribute("name"));
						} else if (thumbType == "imgNum") {
							$thisThumb.find(".thumbTitle").html($thisThumb.index() + adjustIndex);
						} else {
							$thisThumb.find(".thumbTitle").remove();
							...
							if (this.getAttribute("useVidThumb") == "true" && this.getAttribute("src").substr(0,7) != "<iframe") {
								...
								$thisVideo
									.data({
										...
										"dimensions": [$thisThumb.width(), $thisThumb.height() - $thisThumb.find(".thumbTitle").height() - 5],
										"trackMedia": false,
					...
					// only text to appear on thumbnail - removed caption/number only options in xwd as they don't look great
						...
						$thisThumb.find(".thumbMedia").remove();
						$thisThumb.find(".thumbTitle").html(thumbType == "caption" ? this.getAttribute("name") : $thisThumb.index() + adjustIndex);
					}
				}
			});
			...
			$("#thumbHolder .thumbBox:not(.playAll)")
				.click(function(e, playAll) {
					...
					if (chgInfo == true) {
						$("#thumbHolder .current").removeClass("current");
						...
						var showNew = function() {
							...
							if (thisData.nodeName == "image") {
								...
								var src = x_templateLocation + "common_html5/sound.jpg",
									alt = this.getAttribute("tooltip") != undefined ? this.getAttribute("tooltip") : x_currentPageXML.getAttribute("soundTip") == undefined ? "Sound Clip" : x_currentPageXML.getAttribute("soundTip");
							...
							if (thisData.getAttribute("mediaW") != "Full") {
								...
								$mainTxt.html('<h3>' + thisData.getAttribute("name") + '</h3><p>' + x_addLineBreaks(thisData.getAttribute("txt")) + '</p>' + pageLink);
			...
			if ($("#thumbHolder .thumbBox").length == 1 || ($("#thumbHolder .thumbBox").length == 2 && x_currentPageXML.getAttribute("playAll") == "true")) {
				...
				$mainInfoHolder.addClass("noThumbs");
			} else {
		...
		this.setInfoHolderHeight = function() {
			...
			if (x_browserInfo.mobile != true) {
				maxH = $x_pageHolder.height() - (parseInt($x_pageDiv.css("padding-top")) * 2) - (x_currentPageXML.getAttribute("thumbPos") == "grid" ? 0 : $("#thumbBar").height()) - parseInt($mainInfoHolder.css("margin-top")) - parseInt($mainInfoHolder.css("margin-bottom")) - 5;
			} else {
				maxH = $(window).height();
			}
			$mainInfoHolder.data("height", maxH);
			if (x_currentPageXML.getAttribute("thumbPos") != "grid" && $("#thumbHolder .thumbBox").length > 1 && x_browserInfo.mobile != true) {
...
<div id="pageContents">
	...
	<div id="mainInfoHolder">
		<div id="gridBack"><span class="fa fa-x-prev-circle"></span></div>
